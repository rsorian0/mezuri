import LogoSimbol from '../assets/logo-simbolo.svg'
import KunlatekLogo from '../assets/kunlatek.svg'

const SOCIALS = [
  { href: 'https://www.instagram.com/mezuribr', title: 'Instagram', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg> },
  { href: 'https://www.youtube.com/@mezuribr', title: 'YouTube', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M23.5 6.2a3 3 0 0 0-2.1-2.1C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.4.6A3 3 0 0 0 .5 6.2 31.6 31.6 0 0 0 0 12a31.6 31.6 0 0 0 .5 5.8 3 3 0 0 0 2.1 2.1c1.9.6 9.4.6 9.4.6s7.5 0 9.4-.6a3 3 0 0 0 2.1-2.1A31.6 31.6 0 0 0 24 12a31.6 31.6 0 0 0-.5-5.8zM9.75 15.5V8.5l6.25 3.5-6.25 3.5z"/></svg> },
  { href: 'https://www.facebook.com/mezuribr', title: 'Facebook', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.07C24 5.41 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.04V9.41c0-3.02 1.8-4.7 4.54-4.7 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.95.93-1.95 1.88v2.26h3.32l-.53 3.5h-2.79V24C19.61 23.1 24 18.1 24 12.07z"/></svg> },
  { href: 'https://www.linkedin.com/company/mezuribr/', title: 'LinkedIn', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M20.45 20.45h-3.55v-5.57c0-1.33-.03-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.36V9h3.41v1.56h.05c.48-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29zM5.34 7.43a2.06 2.06 0 1 1 0-4.12 2.06 2.06 0 0 1 0 4.12zM3.56 20.45h3.56V9H3.56v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.23.8 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z"/></svg> },
  { href: 'https://www.tiktok.com/@mezuribr', title: 'TikTok', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.32 6.32 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/></svg> },
]
...
const socialStyle = { width: 34, height: 34, border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.45)', textDecoration: 'none', transition: 'all 0.15s' }
    ...
    <footer style={{ all: 'unset', display: 'block', background: '#0f172a', fontFamily: "'Inter', sans-serif", color: 'rgba(255,255,255,0.5)' }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '4rem 2.5rem 3rem', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '3rem' }}>
        <div>
          <a href="#" onClick={e => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }) }} style={{ display: 'inline-block', marginBottom: '1.25rem', textDecoration: 'none' }}>
            <img src={LogoSimbol} alt="mezuri" style={{ height: 36, display: 'block', filter: 'brightness(0) invert(1)' }} />
          </a>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            {SOCIALS.map(s => (
              <a key={s.title} href={s.href} target="_blank" rel="noopener" title={s.title} style={socialStyle}
          ...
          {[['como-funciona','Como funciona'],['funcionalidades','Funcionalidades'],['exclusividade','Para quem é'],['agendar','Agendar diagnóstico']].map(([id, label]) => (
            <div key={id} style={{ marginBottom: '0.6rem' }}>
              <a href={`#${id}`} onClick={e => { e.preventDefault(); scrollTo(id) }}
                style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.45)', textDecoration: 'none', transition: 'color 0.15s', display: 'block' }}
                onMouseOver={e => e.currentTarget.style.color = 'white'}
                onMouseOut={e => e.currentTarget.style.color = 'rgba(255,255,255,0.45)'}>{label}</a>
            </div>
          ))}
        </div>
        <div>
          ...
          {[['https://kunlatek.com.br','Kunlatek',true],['#','Sobre',false],['#','Contato',false]].map(([href, label, external]) => (
        ...
        <div>
          ...
          {[['#','Política de privacidade'],['#','Termos de uso'],['#cookies','Preferências de cookies']].map(([href, label]) => (
            <div key={label} style={{ marginBottom: '0.6rem' }}>
              <a href={href}
                style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.45)', textDecoration: 'none', transition: 'color 0.15s', display: 'block' }}
                onMouseOver={e => e.currentTarget.style.color = 'white'}
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '1.25rem 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <a href="https://kunlatek.com.br/" target="_blank" rel="noopener" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
          ...
          <img src={KunlatekLogo} alt="Kunlatek" style={{ height: 16, display: 'block', filter: 'brightness(0) invert(1)' }} />
        </a>
      </div>
    </footer>
  )
